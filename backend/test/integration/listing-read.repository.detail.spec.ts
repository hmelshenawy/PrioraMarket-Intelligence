  ...
  it('reads listing, marketplace, and latest snapshot without writes', async () => {
    const prisma = {
      listing: { findFirst: jest.fn().mockResolvedValue({
        id: 1, uuid: 'external-1', marketplaceSourceId: 2, title: 'Toyota Camry', make: 'Toyota', model: 'Camry', trim: null, year: 2022,
        price: '85000.00', mileage: 45000, condition: 'used', location: 'Dubai', sellerType: 'dealer', url: 'https://example.test/1',
    ...
    await expect(repository.findDetail({ where: { id: BigInt(1) } })).resolves.toMatchObject({
      id: '1', externalId: 'external-1', marketplace: 'Dubizzle UAE', bodyType: 'sedan', fuel: 'petrol', photosCount: 12
    });
